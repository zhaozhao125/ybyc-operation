<template>
    <div id="car-start-record-table" class="components-content">
        <el-table :data="tableData" height="100%">
            ...
            <el-table-column label="操作类型" show-overflow-tooltip>
                <template slot-scope="scope">
                    <el-tag v-if="scope.row.typeCode ==='accOn'" type="success">点火</el-tag>
                    <el-tag v-else type="danger">熄火</el-tag>
                </template>
            </el-table-column>
...
export default {
    name: "car-start-record-table",
    mixins: [searchPage, position],
    methods: {
        handleSearch() {
            let searchObj = { ...this.paging, ...this.searchData }
            this.$service.get_startRecordPagingOrSearch(searchObj).then(res => {
