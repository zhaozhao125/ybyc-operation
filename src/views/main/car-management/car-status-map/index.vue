    ...
    <map-search @refresh='refresh()' isCityName v-model="params.cityId" :selectWidth='300' :changeTime='changeTime' :loading='timeLoading'>
      ...
      <template slot="select">
        <div class="type-select">
          ...
          <el-select v-model="params.rentTypeCode" placeholder="请选择" clearable class="multi-select" @clear='clearRentTypeCode'>
            <el-option label="分时" value="1"> </el-option>
            <el-option label="短/长租" value="3"> </el-option>
          </el-select>
        </div>
      </template>
    </map-search>
...
export default {
  ...
  methods: {
    ...
    statusMapRentStatus() {
      ...
      this.$service.getStatusMapRentStatus(this.params.cityId, this.params.rentTypeCode).then(res => {
