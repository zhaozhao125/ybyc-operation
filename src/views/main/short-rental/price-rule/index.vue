        ...
        <template slot="ladderPrice" slot-scope="scope">
          <el-table ref="priceTable" :data="ladderPrice" class="price-table" style="width: 100%">
            ...
            <el-table-column prop="minRentDays" label="租期（天）" width="150">
              <template slot-scope="scope">
                <div class="el-form-item" data-name="minRentDays" v-if="scope.$index === 0">
                  <el-input v-model.trim="scope.row.minRentDays" size="mini" style="width:120px" disabled></el-input>
                </div>
                <div class="el-form-item" data-name="minRentDays" v-else>
            ...
            <el-table-column prop="insurancePrice" label="基础服务费（元 /天）" width="180">
              <template slot-scope="scope">
        ...
        Object.keys(priceItem).forEach(item => {
          ...
          if ($item) {
            ...
            if (!priceItem[item] && priceItem[item] !== 0) {
              ...
              errMsg.add('输入值不能为空')
            } else if (!isPositiveInt(priceItem[item])) {
              $item.classList.remove('is-success')
              $item.classList.add('is-error')
              errMsg.add('输入数值必须为大于等于0的整数')
            } else if (item == 'minRentDays') {
            // 租期为整数
              if (Number(priceItem[item]) <= lastRentDay) {
                $item.classList.remove('is-success')
                $item.classList.add('is-error')
                errMsg.add('租期必须大于上一租期')
              } else {
