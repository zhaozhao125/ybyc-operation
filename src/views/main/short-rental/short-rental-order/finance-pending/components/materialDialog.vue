<template>
  <el-dialog
    ...
    width= '500px'
    >
    <el-form class="material_dialog" label-width="90px" labelPosition= 'left'>
      <el-form-item label="已领取物料" v-if="material.materielStatus==='received'" class="oreadyGet">
      ...
      <el-form-item label="选择物料" v-if="material.materielStatus==='unreceived'" >
      ...
      <el-form-item label="已领物料：" v-if="material.materielStatus==='returned'">
         <el-row :gutter="20" v-if="oreadyMaterialList.length">
            <el-col :span="6" v-for="item in oreadyMaterialList" :key="item.id"><div>{{item.materielName}}</div></el-col>
          </el-row>
         <span v-else>无</span>
      </el-form-item>
      <el-form-item label="未还物料：" v-if="material.materielStatus==='returned'">
         <el-row :gutter="20" v-if="unreturn.length">
            <el-col :span="6" v-for="item in unreturn" :key="item.id"><div>{{item.materielName}}</div></el-col>
          </el-row>
         <span v-else>无</span>
      </el-form-item>
    </el-form>
