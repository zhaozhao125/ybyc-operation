  ...
  <div class="components-content">
    <el-table :data="list" class="table-container" height="100%">
      ...
      <el-table-column
        ...
        width="180">
          <template slot-scope="scope">
            <span class="state-yellow">{{scope.row.orderStatusName}}</span>
          </template>
      </el-table-column>
      <el-table-column
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <!-- <div>
             <el-button type = "text" v-if="scope.row.userName" @click = "$emit('customer-detail', scope.row.userId)">{{scope.row.userName}}</el-button>
          </div>
          <div><span>{{scope.row.userPhone}}</span></div> -->
          <div>
      ...
      <el-table-column
        ...
        label="预计取车时间">
        <template slot-scope="scope">
          <span>{{scope.row.expectTakeTime | timeFilter}}</span> <br>
        </template>
      </el-table-column>
      <el-table-column
        prop="expectTakeTime"
