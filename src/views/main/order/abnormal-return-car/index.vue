    ...
    <el-card class="table-box">
      ...
      <div class="table-container">
        <el-table :data="tableData" height="100%">
          <el-table-column label="订单信息" min-width="210px">
            <template slot-scope="scope">
              <span v-if="!scope.row.orderSn">未知</span>
              <el-button type="text" v-else @click="handleDetails(scope.row)">{{scope.row.orderSn}}</el-button>
            </template>
          </el-table-column>
          ...
          <el-table-column prop="stationName" label="提交时手机所在网点" min-width="220px">
          </el-table-column>
          <el-table-column label="提交异常时间" min-width="180px">
            <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    loadTableData() {
      ...
      this.$service.abnormalReturnCarList(params).then(res => {
        this.tableData = res.data.data.records
        this.tableData.forEach(item => {
          item.abnormalImgList = [item.parkImg, item.stationImg, item.otherImg].filter(img => img !== null)
        })
    ...
    orderHistoryTrack(row) {
      this.sendOrderSn(row.orderSn, 'carTrack')
    },
    ...
    viewImg(row) {
      console.log(row.abnormalImgList)
      imgViewer.viewImg(row.abnormalImgList)
    }
  }
}
</script>
