<template>
  <section class="all-website">
    <el-select v-model="currentValue" @change="selectChange" @clear='clearItem' @input="handleInput" :remote-method="remoteMethod" :no-data-text='noDataText' :loading="loading" :remote='isRequest?true:false' :filterable='isRequest?true:false' :clearable="clearable" :placeholder="placeholder" default-first-option :value-key="integratedValue?'value':null" :disabled="disabled">
    ...
    remoteMethod: _debounce(function (name) {
      if (name !== '') {
        ...
        this.$service.getChargingPileNetworkStatus(params).then(res => {
          if (res.data.code == 0) {
            this.newListData = this.$service.formateChargingPileStationNameWithname(res.data.data.records)
          }
