<template>
  <section class="all-website">
    <el-select v-model="stationName?stationName:currentValue" ref="selectOb" @change="selectChange" @clear='clearItem' @input="handleInput" :remote-method="remoteMethod" :no-data-text='noDataText' :loading="loading" :remote='isRequest?true:false' :filterable='isRequest?true:false' :clearable="clearable" :placeholder="placeholder" default-first-option :value-key="integratedValue?'value':null" :disabled="disabled">
...
export default {
  ...
  methods: {
    ...
    remoteMethod: _debounce(function (name) {
      if (name !== '') {
        this.loading = true
        this.$service.post_allChargingStation(name, null).then(res => {
