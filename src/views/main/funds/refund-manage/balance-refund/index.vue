<template>
  ...
  <div class='finance_check'>
    ...
    <v-page :visible.sync="addOrEditPage"
      ...
      <template slot="title"
                class='check_title'>
        <h3 style="line-height: 30px; display: inline-block">余额退款管理</h3>
        <div style="float: right"
             v-if="information.status==='checkSuccess'">
          <el-button size='small'
                     @click="financeCancel">取消</el-button>
      ...
      <template slot="content">
        <check-information :infor='information'
                           @on-reload="backIndex"
                           @getStatus="financeStatus"
                           :searchData="searchData"></check-information>
      </template>
    </v-page>
...
export default {
  ...
  methods: {
    ...
    handleSearch(data) {
      ...
      if (this.searchData.add || this.searchData.handle) {
        let timeDuring = this.searchData.add
          ? this.searchData.add
          : this.searchData.handle
        this.searchData.dateStart = handleDate(timeDuring[0])
        this.searchData.dateEnd =
          handleDate(timeDuring[1]).slice(0, 11) + '23:59:59'
