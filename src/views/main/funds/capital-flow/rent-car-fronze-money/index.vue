<template>
  <!-- 长租违章押金 -->
  <div id="longRentViolationDeposit">
    <el-card class="table-box">
      ...
      <div class="table-container">
        <el-table :data="tableData"
          ...
          <el-table-column prop="longTermRentViolationDepositBefore"
                           label="发生前余额"
                           min-width="140"></el-table-column>
          <el-table-column prop="longTermRentViolationDeposit"
                           label="发生后余额"
                           min-width="140"></el-table-column>
          ...
          <el-table-column prop="paymentTypeText"
                           label="支付方式"></el-table-column>
          <el-table-column prop="evidenceNote"
                           label="凭证"
                           min-width="260">
            <template slot-scope="scope">
...
export default {
  ...
  created() {
    this.initSubject('longTermRentViolationDeposit')
    this.loadTableData()
  },
  methods: {
    loadFundsSum() {
    ...
    download(obj, s, e) {
      ...
      if (len === 0) {
        ...
        if (end - start <= 31 * 24 * 60 * 60 * 1000) {
          ...
          this.$message.warning('导出数据时间范围必须小于等于31天，请重新设置')
        }
      }
    },
    exportFile() {
    ...
    formateSubjects(data = []) {
      ...
      this.searchSettings[3].options = tmp
    },
    handleSearch(data) {
      ...
      if (searchData.datetimerange && searchData.datetimerange.length !== 0) {
    ...
    loadTableData() {
      ...
      this.$service.getCapitalFlowBalance(this.searchData).then(res => {
        let total = 0
...
<style lang="scss">
#longRentViolationDeposit .el-form-item__label {
  width: 100px;
}
</style>
